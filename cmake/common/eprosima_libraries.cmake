...
# Macro to find thirdparty libraries and build them if has CMakeLists.txt.
#
# Arguments:
...
macro(eprosima_find_package package thirdparty_name)
    # Parse arguments.
    ...
    if(THIRDPARTY_${package} STREQUAL "OFF")
    ...
    if((THIRDPARTY_${package} STREQUAL "FORCE") OR ((NOT ${package}_FOUND) AND (THIRDPARTY_${package} STREQUAL "ON")))
        ...
        if(EXISTS "${PROJECT_SOURCE_DIR}/thirdparty/${thirdparty_name}/CMakeLists.txt")
            set(SUBDIRECTORY_EXIST TRUE)
            set(CMAKE_PACKAGE_SUBDIRECTORY "thirdparty/${thirdparty_name}")
        elseif(EXISTS "${PROJECT_SOURCE_DIR}/thirdparty/${thirdparty_name}/${package}/CMakeLists.txt")
            # Specific case for miniupnpc that is under miniupnp/miniupnpc
            set(SUBDIRECTORY_EXIST TRUE)
            set(CMAKE_PACKAGE_SUBDIRECTORY "thirdparty/${thirdparty_name}/${package}")
        endif()

        message(STATUS "Thirdparty package to build ${CMAKE_PACKAGE_SUBDIRECTORY}")
        ...
        if(SUBDIRECTORY_EXIST)
            foreach(opt_ ${FIND_OPTIONS})
                set(${opt_} ON)
            endforeach()
            add_subdirectory(${PROJECT_SOURCE_DIR}/${CMAKE_PACKAGE_SUBDIRECTORY})
            set(${package}_FOUND TRUE)

            message(STATUS "Found ${package}: ${PROJECT_SOURCE_DIR}/${CMAKE_PACKAGE_SUBDIRECTORY}")
        endif()

        set(CMAKE_PREFIX_PATH ${PROJECT_SOURCE_DIR}/${CMAKE_PACKAGE_SUBDIRECTORY} ${CMAKE_PREFIX_PATH})
        set(CMAKE_PREFIX_PATH ${PROJECT_SOURCE_DIR}/${CMAKE_PACKAGE_SUBDIRECTORY}/${thirdparty_name} ${CMAKE_PREFIX_PATH})
...
# Macro to find all thirdparty libraries without building them.
#
# Arguments:
